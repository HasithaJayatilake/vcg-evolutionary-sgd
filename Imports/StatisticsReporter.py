...
class StatisticsReporter(BaseReporter):
    ...
    the most-fit genomes and information on genome/species fitness and species sizes.
    """
    def __init__(self, generation_interval=20, filename_prefix='neat-stats-checkpoint-'):
        BaseReporter.__init__(self)
        self.most_fit_genomes = []
        self.generation_statistics = []
        self.generation_vcg_stats = []
    ...
    def end_generation(self, config, population, species_set, upload_drive=None):
        ...
        if (checkpoint_due) and (upload_drive is not None):
            self.save_checkpoint(self.current_generation, upload_drive)
            self.last_generation_checkpoint = self.current_generation

    def save_checkpoint(self, generation, upload_drive):
        """ Save the current simulation state. """
        filename = '{0}{1}'.format(self.filename_prefix, generation)
        print("Saving checkpoint to {0}".format(filename))
        data = self.most_fit_genomes

        with gzip.open(filename, 'w', compresslevel=5) as f:
            pickle.dump(data, f, protocol=pickle.HIGHEST_PROTOCOL)

        folder_id = '1NOJDM5SEL-gzscELPwe9K5oxSSxK_Ehy'         
    ...
    def restore_checkpoint(self, filename):
        ...
        try:
            with gzip.open(filename) as f:
                self.most_fit_genomes = pickle.load(f)
            return 0
